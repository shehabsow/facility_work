...
def load_change_log():
    if os.path.exists('change_log.csv'):
        return pd.read_excel('change_log.xlsx', encoding='xlsxwriter')
    return pd.DataFrame(columns=[
...
def save_checklist_data(df):
    df.to_excel('checklist_records.xlsx', index=False, encoding='xlsxwriter')

def save_change_log(df):
    df.to_csv('change_log.xlsx', index=False, encoding='xlsxwriter')
